...
(let ((out *standard-output*))
  (cffi:defcallback godot-extension-init :uint8 ((proc-addr-getter :pointer)
                                                 (ext-class-lib :pointer)
                                                 (gdext-init-record :pointer))
    (format out "~&Got in: ~A ~A ~A"
            proc-addr-getter ext-class-lib gdext-init-record)
    1))
  ...
  (let ((exec-path (namestring
    ...
    (cffi:with-foreign-string (exec-path-ptr exec-path)
      (cffi:with-foreign-object (argv :pointer)
        ...
        (let ((instance (%gdext:create-godot-instance 1 argv (cffi:callback godot-extension-init))))
